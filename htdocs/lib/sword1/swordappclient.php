...
require("sword1/swordappservicedocument.php");
require("sword1/swordappentry.php");
require("sword1/swordapperrordocument.php");
require("sword1/swordapplibraryversion.php");
...
class SWORDAPPClient {
	...
	function servicedocument($sac_url, $sac_u, $sac_p, $sac_obo) {
		...
		$sac_curl = curl_init();
		/*
		define('HTTP_PROXY_HOST', 'wwwcache.arts.ac.uk');
		define('HTTP_PROXY_PORT', '3128');
		if (defined('HTTP_PROXY_HOST') && HTTP_PROXY_HOST != '') {
		    curl_setopt($sac_curl, CURLOPT_PROXY, HTTP_PROXY_HOST);
		}
		if (defined('HTTP_PROXY_PORT') && HTTP_PROXY_PORT != '') {
		    curl_setopt($sac_curl, CURLOPT_PROXYPORT, HTTP_PROXY_PORT);
		}
		*/
		...
		if ($sac_status == 200) {
			try {
                ...
                throw new ExportException('exportsword', "Error parsing service document (" . $e->getMessage() . ")");
            }
		} else {
	...
	function deposit($sac_url, $sac_u, $sac_p, $sac_obo, $sac_fname, 
		...
		*/
		...
		$headers = array();
		global $sal_version;
		array_push($headers, "User-Agent: SWORDAPP PHP library (version " . $sal_version . ") " . "http://php.swordapp.org/");
		...
		if ($sac_noop == true) {
			array_push($headers, "X-No-Op: true");
	        	...
	        	array_push($headers, "X-No-Op: false");
	        }
		if ($sac_verbose == true) {
		...
		if (($sac_status >= 200) && ($sac_status < 300)) {
			try {
			    ...
			    throw new ExportException('exportsword', 'Error parsing response entry (' . $e->getMessage() . ')'
			    						 . '<br />The status code from the server was ' . $sac_status
			    						 . '<br />The server returned this message: <pre>' . $sac_resp . '</pre>'
			    						 );
			}
		} else {
			try {
			    ...
			    throw new ExportException('exportsword', 'Error parsing error document (' . $e->getMessage() . ')'
			    						 . '<br />The status code from the server was ' . $sac_status
			    						 . '<br />The server returned this message: <pre>' . $sac_resp . '</pre>'
			    						 );
			}
